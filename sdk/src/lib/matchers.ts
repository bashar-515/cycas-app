...
import { CycasDefaultError } from "../models/errors/cycasdefaulterror.js";
...
export function match<T, E>(
...
): MatchFunc<T, E | CycasDefaultError | ResponseValidationError> {
  return async function matchFunc(
    ...
    [
      result: Result<T, E | CycasDefaultError | ResponseValidationError>,
      raw: unknown,
    ]
  > {
    let raw: unknown;
    ...
    if (!matcher) {
      return [{
        ok: false,
        error: new CycasDefaultError("Unexpected Status or Content-Type", {
          response,
          request,
    ...
    if (matcher.enc === "fail") {
      return [{
        ok: false,
        error: new CycasDefaultError("API error occurred", {
          request,
          response,
          body,
        }),
